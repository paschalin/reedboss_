...
<div class="@if ($disabled) opacity-60 @endif">
  ...
  <div class="relative rounded-md @unless($shadowless) shadow-sm @endunless">
    ...
    <textarea {{ $attributes->class([$getInputClasses($hasError)])->merge(['autocomplete' => 'off']) }} x-data="{
        resize() {
            $el.style.height = '{{ $height }}px';
            $el.style.height = (($el.scrollHeight < {{ $height }} ? {{ $height }} : $el.scrollHeight) + 2) + 'px';
        }
    }" @input="resize()"
      x-init="resize()" x-effect="resize()" :style="{ height: '{{ $height }}px' }">{{ $slot }}</textarea>
