<div>
  <div class="max-w-7xl mx-auto pb-10 sm:px-6 lg:px-8">
    <x-jet-form-section submit="save">
      <x-slot name="title">
        {{ $k_b_category->id ? __('Edit Knowledge Base Category') : __('New Knowledge Base Category') }}
      </x-slot>

      <x-slot name="description">
        <div>{{ __('Please fill the form and save.') }}</div>
        <x-link type="button" class="mt-6" :href="route('knowledgebase.categories')">{{ __('List Knowledge Base Category') }}</x-link>
      ...
      <x-slot name="form">
        ...
        <div class="col-span-6">
          <div x-data="{ imageName: null, imagePreview: null }">
            ...
            <!-- Current Image -->
            @if ($k_b_category['image'] ?? null)
              <div class="mt-2" x-show="! imagePreview">
                <img src="{{ $k_b_category['image'] }}" alt="{{ $k_b_category['name'] }}" class="rounded-full h-20 w-20 object-cover">
              </div>
            @endif
        ...
        <div class="col-span-6 sm:col-span-3">
          <x-ui-input :label="__('Name')" wire:model.defer="k_b_category.name" />
        </div>

        <!-- Slug -->
        <div class="col-span-6 sm:col-span-3">
          <x-ui-input :label="__('Slug')" wire:model.defer="k_b_category.slug" />
        </div>

        <!-- Order -->
        <div class="col-span-6 sm:col-span-3">
          <x-ui-input :label="__('Order Number')" wire:model.defer="k_b_category.order_no" />
        </div>

        <!-- Title -->
        <div class="col-span-6 sm:col-span-3">
          <x-ui-input :label="__('Page Title')" wire:model.defer="k_b_category.title" />
        </div>

        <!-- Description -->
        <div class="col-span-6">
          <x-textarea :label="__('Meta Description')" wire:model.defer="k_b_category.description" />
        </div>
        ...
        @if ($mainCategories)
          <div class="col-span-6">
            <x-ui-select :label="__('Parent Category')" :options="$mainCategories" option-value="id" option-label="name"
              wire:model.defer="k_b_category.k_b_category_id">
            </x-ui-select>
          </div>
        @endif
        ...
        <div class="col-span-6">
          <label for="active" class="inline-flex items-center">
            <x-checkbox id="active" wire:model.defer="k_b_category.active" />
