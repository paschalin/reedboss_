<div>
  <x-slot name="title">
    {{ __('Followings of :user', ['user' => $user->displayName]) }}
  </x-slot>
  ...
  @if ($followings->isEmpty())
    <div class="text-center py-12">
      ...
      <p class="mt-1">{{ __(':user does not have any followings yet.', ['user' => $user->displayName]) }}</p>
      {{-- <div class="mt-6">
    ...
    <ul role="list" class="grid grid-cols-1 gap-6 sm:grid-cols-2">
      @foreach ($followings as $following)
        <li class="col-span-1 rounded-lg bg-white dark:bg-gray-900 shadow p-2">
          <x-profile-overview :user="$following" less="1" />
        </li>
      @endforeach
    </ul>
  @endif
